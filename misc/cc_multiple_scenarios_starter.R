...
# Starter file with the previous set of scenarios.  
...
run_SSMSE_dir <- file.path("testing_cloud")

# scenario names
default <- file.path(run_SSMSE_dir, "default")
...
#make a copy of sample_structure
sample_struct_fixed <- sample_struct
...
sample_struct_fixed$FixedCatch <- sample_struct_fc$FixedCatch
sample_struct_fixed$FixedCatch <- sample_struct_fixed$FixedCatch %>% 
  filter(FltSvy == 5) 
sample_struct_fixed$FixedCatch <- sample_struct_fixed$FixedCatch %>%
  mutate(Catch = if_else(FltSvy == 5 & Yr == 2025, .7, Catch))

#Add fixed catches in the EM
sample_struct_fixed$FixedCatchEM <- sample_struct_fc$FixedCatchEM
sample_struct_fixed$FixedCatchEM <- sample_struct_fixed$FixedCatchEM %>% 
  filter(FltSvy == 5) 
sample_struct_fixed$FixedCatchEM <- sample_struct_fixed$FixedCatchEM %>%
  mutate(Catch = if_else(FltSvy == 5 & Yr == 2025, .7, Catch))

#Add random red tide events 

add_random_red_tide_events <- function(
  ...
  set.seed(12345)
  rt_year <- sample(min(sample_struct_copy$catch$Yr):max(sample_struct_copy$catch$Yr), rt_events)
...
sample_struct_random_10 <- add_random_red_tide_events(sample_struct = sample_struct, rt_events = 10)
sample_struct_random_25 <- add_random_red_tide_events(sample_struct = sample_struct, rt_events = 25)

#Add regularly scheduled red tide events 

add_regular_red_tide_events <- function(
    sample_struct = sample_struct,
    rt_fleet = 5,
    rt_mortality = 0.2,
    rt_intervals = 1
  ...
  sample_struct_copy <- sample_struct
  rt_year <- seq(min(sample_struct_copy$catch$Yr), max(sample_struct_copy$catch$Yr), by = rt_intervals)
...
sample_struct_regular_3 <- add_regular_red_tide_events(sample_struct = sample_struct, rt_intervals = 3)
sample_struct_regular_5 <- add_regular_red_tide_events(sample_struct = sample_struct, rt_intervals = 5)

sample_struct_regular_5_mortality_5 <- add_regular_red_tide_events(sample_struct = sample_struct, rt_mortality = 0.5, rt_intervals = 5)


# create a list of sample structures for each OM/MP run. 
sample_struct_list_all <- list(
  "default" = sample_struct,
  "red_tide_random_10" = sample_struct_random_10,
  "red_tide_random_25" = sample_struct_random_25,
  "red_tide_regular_3" = sample_struct_regular_3,
  "red_tide_regular_5" = sample_struct_regular_5,
  "red_tide_regular_5_mortality_5" = sample_struct_regular_5_mortality_5
)
...
run_res_path <- file.path(run_SSMSE_dir, "results_SigmaR")
dir.create(run_res_path)
res<-SSMSE::run_SSMSE(
  scen_name_vec=c("default", "red_tide_random_10", "red_tide_random_25", "red_tide_regular_3", "red_tide_regular_5", "red_tide_regular_5_mortality_5"), # name of the scenario
  out_dir_scen_vec = run_res_path, # directory in which to run the scenario
  iter_vec = c(10,10,10,10,10,10), # run with X iterations each
  OM_name_vec = NULL, # specify directories instead
  OM_in_dir_vec = c(normalizePath(default), normalizePath(default), normalizePath(default), normalizePath(default), normalizePath(default), normalizePath(default)), # OM files
  EM_name_vec = NULL, # specify directories instead
  EM_in_dir_vec = c(normalizePath(default), normalizePath(default), normalizePath(default), normalizePath(default), normalizePath(default), normalizePath(default)), # EM files
  run_EM_last_yr = TRUE,
  MS_vec = c("EnvirEM", "EnvirEM", "EnvirEM", "EnvirEM", "EnvirEM", "EnvirEM"), # The management strategy is specified in the EM
  nyrs_vec = c(projyrs, projyrs, projyrs, projyrs, projyrs, projyrs), # Years to project OM forward
  nyrs_assess_vec = c(3, 3, 3, 3, 3, 3), # Years between assessments
...
saveRDS(res, file = file.path(run_SSMSE_dir, "results_SigmaR.rda"))
summary <- SSMSE::SSMSE_summary_all(run_res_path)
saveRDS(summary, file = file.path(run_SSMSE_dir, "results_summary_SigmaR.rda"))

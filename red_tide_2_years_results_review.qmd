...
EM_means_re <- summary$ts %>%
  filter(str_detect(model_run, "_EM_2020")) %>%
  rowwise() %>%
  mutate(
...
ratio_R0 <- summary$scalar %>%
  ...
  mutate(r0 = exp(SR_LN_R0)) %>%
  select(!SR_LN_R0) %>%
  filter(model_run %in% c(paste0(OM_name, "_OM"), paste0(OM_name, "_EM_2020"))) %>%
  pivot_wider(
      names_from = model_run,  # this will create "OM" and "EM" columns
      values_from = r0
  ) %>%
  rename("OM" = paste0(OM_name, "_OM") , "EM" = paste0(OM_name, "_EM_2020")) %>%
...
ratio_df %>%
  filter(str_detect(model_run, "_EM_2020") | str_detect(model_run, "_OM")) %>%
  mutate(
    em_model = ifelse(
...
summary$ts %>%
  filter(model_run %in% c(paste0(OM_name, "_OM"), paste0(OM_name, "_EM_2020")), year %in% c(2005, 2014, 2018, 2021)) %>%
  ggplot(aes(factor(year), F_5)) + 
  geom_boxplot(aes(color = model_run)) + 
  facet_wrap(~scenario) +
  ggtitle("model_run = 2020")
```
...
summary$ts %>%
  ...
  rename(
    'EM_2020' = paste0(OM_name, "_EM_2020"),
    'OM' = paste0(OM_name, "_OM")
  ) %>%
  mutate(Residual = EM_2020 - OM) %>%
  kable(
    # Rename columns directly within kable

...
# This version uses the _merged data with all scenarios
# This version may be broken and require more work to run all the way through 
# because of commented code and changes in years from scenario to scenario.  
...
if(save == TRUE){
  ggsave(file.path(run_SSMSE_dir,"plots_all", "ts_mean_F_5_core.png"),
...
if(save == TRUE){
  ggsave(file.path(run_SSMSE_dir,"plots_all", "ts_mean_F_5_all_yrs.png"),
...
if(save == TRUE){
  ggsave(file.path(run_SSMSE_dir,"plots_all", "ts_mean_F_5_sel_rt_2.png"),
         width = 8, height = 6, units = "in", device = "png")
}
...
if(save == TRUE){
  ggsave(file.path(run_SSMSE_dir,"plots_all", "ts_mean_F_5_sel_all_yrs.png"),
         width = 8, height = 6, units = "in", device = "png")
}
...
if(save == TRUE){
  save_kable(kable_core, file = file.path(run_SSMSE_dir,"plots_all","core_kable.html"), max_year)
}
...
if(save == TRUE){
  save_kable(kable_all_yrs, file = file.path(run_SSMSE_dir,"plots_all","all_years_kable.html"))
}
...
if(save == TRUE){
  save_kable(kable_sel_rt_2, file = file.path(run_SSMSE_dir,"plots_all","sel_rt_2_kable.html"))
}
...
if(save == TRUE){
  save_kable(kable_sel_all_yrs, file = file.path(run_SSMSE_dir,"plots_all","sel_all_yrs_kable.html"))
}

#### Selectivity rt_34

kable_sel_rt_34 <- create_residual_kable(min_year, max_year_short_term, selectivity_rt_34, max_year_extra)
kable_sel_rt_34

if(save == TRUE){
  save_kable(kable_sel_rt_34, file = file.path(run_SSMSE_dir,"plots_all","sel_rt_34_kable.html"))
}

#### Selectivity 34_all_yrs

kable_sel_34_all_yrs <- create_residual_kable(min_year, max_year_short_term, selectivity_34_all_yrs, max_year_extra)
kable_sel_34_all_yrs

if(save == TRUE){
  save_kable(kable_sel_34_all_yrs, file = file.path(run_SSMSE_dir,"plots_all","sel_34_all_yrs_kable.html"))
}
...
if(save == TRUE){
  save_kable(kable_core, file = file.path(run_SSMSE_dir,"plots_all","core_longterm_kable.html"))
}
...
if(save == TRUE){
  save_kable(kable_all_yrs, file = file.path(run_SSMSE_dir,"plots_all","all_years_longterm_kable.html"))
}
...
if(save == TRUE){
  save_kable(kable_sel_rt_2, file = file.path(run_SSMSE_dir,"plots_all","sel_rt_2_longterm_kable.html"))
}
...
if(save == TRUE){
  save_kable(kable_sel_all_yrs, file = file.path(run_SSMSE_dir,"plots_all","sel_all_yrs_longterm_kable.html"))
}
...
# model_run_type = the end year of the model run (i.e. "2116") or "_OM"
create_reccom_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  summary$ts %>%
    ...
    ggtitle(paste0("DeadB from ", min_year, "-", max_year, " for each fleet - ", scenario_name)) + xlab("OM name") + ylab("Mean Removals (mt)") +
...
create_reccom_plot_self_ref <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  dynamic_medians <- plot_data %>%
    ...
    group_by(fleet_name, em_name) %>%
    summarise(median_deadB = median(mean_deadB), .groups = "drop")
  
  title <- paste0("DeadB_", min_year, "_", max_year, "_", scenario_name, "_", model_run_type)
  
  p <- ggplot(plot_data, aes(om_name, mean_deadB)) + 
    geom_boxplot() +
  ...
  if(save == TRUE){
    ggsave(file.path(run_SSMSE_dir, "plots_all", paste0(title,"_self_ref", ".png")),
...
create_reccom_om_em_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  EM_runs %>%
    ...
    geom_boxplot()+
    facet_grid(type_name~em_name, scales = "free_x")+
    theme_bw() +
    theme(
      axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)
    ) + 
    ggtitle(paste0("DeadB from ", min_year, "-", max_year, " for each fleet - ", scenario_name)) + xlab("OM name") + ylab("OM:EM Ratio") +
...
create_reccom_om_em_plot_self_ref <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  dynamic_medians <- plot_data %>%
    filter(as.character(om_name) == as.character(em_name)) %>%
    group_by(type_name, em_name) %>%
    summarise(median_val = median(mean_om_em), .groups = "drop")
  
  # 3. Plotting
  title <- paste0("DeadB_", min_year, "_", max_year, "_", scenario_name, "_", model_run_type, "_OMEM")
  
  p <- ggplot(plot_data, aes(om_name, mean_om_em)) + 
    geom_boxplot() +
...
create_deadb_bias_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  EM_runs %>%
    ...
    ggtitle(paste0("DeadB from ", min_year, "-", max_year, " for each fleet - ", scenario_name)) + xlab("OM name") + ylab("% Bias") +
...
create_deadb_bias_plot_self_ref <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  p <- ggplot(plot_data, aes(om_name, mean_bias)) + 
    geom_boxplot() +
  ...
  if(save == TRUE){
    ggsave(file.path(run_SSMSE_dir, "plots_all", paste0(title, "self_ref.png")),
           plot = p, width = 8, height = 6, units = "in", device = "png")
  }
...
create_recruitment_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
...
summary$ts %>%
  ...
  ggtitle(paste0("Recruits from ", min_year, "-", max_year, " for each fleet - ", scenario_name)) + xlab("OM name") + ylab("Mean Recruits (mt)") +
...
create_recruit_om_em_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  EM_runs %>%
    ...
    ggtitle(paste0("Recruitment from ", min_year, "-", max_year, " for each fleet - ", scenario_name)) + xlab("OM name") + ylab("OM:EM Ratio") +
    geom_hline(yintercept = 1, linetype = "dashed") # switch if you want the median line of the rt_2_x_rt_2
  #geom_hline(data = median_rt_2, aes(yintercept = median_om_em), linetype = "dashed")
...
create_recruitment_bias_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  EM_runs %>%
    ...
    ggtitle(paste0("Recruitment from ", min_year, "-", max_year, " - ", scenario_name)) + xlab("OM name") + ylab("% Bias") +
    geom_hline(yintercept = 1, linetype = "dashed") # switch if you want the median line of the rt_2_x_rt_2
  #geom_hline(data = median_rt_2, aes(yintercept = median_om_em), linetype = "dashed")
  if(save == TRUE){
    ggsave(file.path(run_SSMSE_dir, "plots_all", paste0(title, ".png")),
...
create_ssb_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
...
summary$ts %>%
  ...
  ggtitle(paste0("SSB from ", min_year, "-", max_year, " for each fleet - ", scenario_name)) + xlab("OM name") + ylab("Mean SSB (mt)") +
...
create_ssb_plot_self_ref <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  p <- ggplot(plot_data, aes(om_name, mean_ssb)) + 
    geom_boxplot() +
...
create_ssb_plot(2022, 2042, selectivity_rt_2, "Selectivity 2 years")
create_ssb_plot_self_ref(2022, 2042, selectivity_rt_2, "Selectivity 2 years")
create_ssb_plot(2022, 2042, selectivity_rt_2, "Selectivity 2 years", "_OM")
create_ssb_plot_self_ref(2022, 2042, selectivity_rt_2, "Selectivity 2 years", "_OM")
...
create_ssb_plot(2022, 2042, selectivity_all_yrs, "Selectivity all years")
create_ssb_plot_self_ref(2022, 2042, selectivity_all_yrs, "Selectivity all years")
create_ssb_plot(2022, 2042, selectivity_all_yrs, "Selectivity all years", "_OM")
create_ssb_plot_self_ref(2022, 2042, selectivity_all_yrs, "Selectivity all years", "_OM")


# SSB OM/EM -------------------------------------------------------

create_ssb_om_em_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  EM_runs %>%
    ...
    ggtitle(paste0("SSB from ", min_year, "-", max_year, " for each fleet - ", scenario_name)) + xlab("OM name") + ylab("OM:EM Ratio") +
    geom_hline(yintercept = 1, linetype = "dashed") # switch if you want the median line of the rt_2_x_rt_2
  #geom_hline(data = median_rt_2, aes(yintercept = median_om_em), linetype = "dashed")
  if(save == TRUE){
    ggsave(file.path(run_SSMSE_dir, "plots_all", paste0(title, ".png")),
...
create_ssb_om_em_plot_self_ref <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
...
create_ssb_bias_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  EM_runs %>%
    ...
    ggtitle(paste0("SSB from ", min_year, "-", max_year, " - ", scenario_name)) + xlab("OM Name") + ylab("% Bias") +
    geom_hline(yintercept = 0, linetype = "dashed") # switch if you want the median line of the rt_2_x_rt_2
  #geom_hline(data = median_rt_2, aes(yintercept = median_om_em), linetype = "dashed")
  if(save == TRUE){
    ggsave(file.path(run_SSMSE_dir, "plots_all", paste0(title, ".png")),
...
create_ssb_bias_plot(2022, 2042, selectivity_rt_2, "Selectivity 2 years")


#### Selectivity all_yrs

create_ssb_bias_plot(min_year, max_year_short_term, selectivity_all_yrs, "Selectivity all years")
create_ssb_bias_plot(2022, 2042, selectivity_all_yrs, "Selectivity all years")
...
create_relative_ssb_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  
  base_ssb_unfished <- summary$scalar %>% 
    filter(str_detect(model_run, model_run_type)) %>%
    select(scenario, iteration, SSB_Unfished)
  
  median_rt_2_rec <- summary$ts %>% filter(scenario == "rt_2_x_rt_2", str_detect(model_run, model_run_type), year >= min_year, year <= max_year) %>%
    left_join(base_ssb_unfished, join_by(scenario, iteration)) %>%
    group_by(iteration) %>%
    mutate(mean_ssb = mean(SpawnBio)/SSB_Unfished) %>%
    ungroup()%>%
    reframe(median_ssb = median(mean_ssb))
  
  title <- paste0("Relative_SSB_", min_year, "_", max_year, "_", scenario_name, "_", model_run_type)
  summary$ts %>%
    filter(scenario %in% c(scenario_list, "rt_2_x_rt_2", "rt_2_x_no_rt")) %>%
    filter(str_detect(model_run, model_run_type), year >= min_year, year <= max_year) %>%
    left_join(base_ssb_unfished, join_by(scenario, iteration)) %>%
    group_by(scenario, iteration) %>%
    reframe(mean_ssb = mean(SpawnBio)/SSB_Unfished) %>%
    mutate(om_name = if_else(scenario == "no_rt", "no_rt", if_else(str_extract(scenario, "\\w+(?=_x_)") %in% c("rt_2", "no_rt", "rep_3"), scenario, str_extract(scenario, "\\w+(?=_x_)"))), 
           om_name = fct_relevel(om_name, "reference", "young", "mid", "old", "flat"),
           em_name = str_extract(scenario, "(?<=_x_).*?(?=_all_yrs|_rt_2)"),
           em_name = if_else(scenario %in% all_years, "all years", replace_na(em_name, "reference")), 
    ...
    ggtitle(paste0("Relative SSB from ", min_year, "-", max_year, " for each fleet - ", scenario_name)) + xlab("OM name") + ylab("Relative SSB") +
    geom_hline(data = median_rt_2_rec, aes(yintercept = median_ssb), linetype = "dashed")
  
  if(save == TRUE){
    ggsave(file.path(run_SSMSE_dir, "plots", paste0(title, ".png")),
           width = 8, height = 3, units = "in", device = "png")
  }
}

create_relative_ssb_plot(min_year, max_year_short_term, selectivity_rt_2, "Selectivity 2 years")
create_relative_ssb_plot(min_year, max_year_short_term, selectivity_all_yrs, "Selectivity all years")

create_relative_ssb_plot_self_ref <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  p <- ggplot(plot_data, aes(om_name, mean_ssb)) + 
    ...
    labs(
      title = paste0("Relative SSB from ", min_year, "-", max_year, " - ", scenario_name),
      x = "OM name", 
      y = "Relative SSB (mt)"
    )
...
create_relative_ssb_plot_self_ref(min_year, max_year_short_term, selectivity_rt_2, "Selectivity 2 years")
create_relative_ssb_plot_self_ref(min_year, max_year_short_term, selectivity_all_yrs, "Selectivity all years")
...
create_bratio_plot <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  b_ratio_plot <- EM_runs_dq %>%
    ...
    left_join(OM_runs_dq, by = c("year", "scenario", "iteration"), suffix = c("_em", "_om")) %>%
    group_by(scenario, iteration, year) %>%
    mutate(
      res_bratio = (Value.Bratio_em-Value.Bratio_om)/Value.Bratio_om*100,
    ) %>%
    pivot_longer(
      cols = starts_with("res_"),
      names_to = "type",
...
create_bratio_plot(min_year, max_year_short_term, selectivity_rt_2, "Selectivity 2 years")
create_bratio_plot(2022, 2042, selectivity_rt_2, "Selectivity 2 years")


#### Selectivity all_yrs

create_bratio_plot(min_year, max_year_short_term, selectivity_all_yrs, "Selectivity all years")
create_bratio_plot(2022, 2042, selectivity_all_yrs, "Selectivity all years")

# DeadN Percent Bias -------------------------------------------------------

create_deadn_bias_plot_self_ref <- function (min_year, max_year, scenario_list, scenario_name, model_run_type = "2116") {
  ...
  plot_data <- EM_runs %>%
    ...
    rowwise() %>%
    mutate(commercial = sum(deadN_1, deadN_2), recreational = deadN_4) %>%
    left_join(OM_runs, by = c("year", "scenario", "iteration"), suffix = c("_em", "_om")) %>%
    group_by(scenario, iteration, year) %>%
    mutate(
      com_om = sum(deadN_1_om, deadN_2_om),
      # Calculate % Bias: (EM - OM) / OM * 100
      res_com = (commercial - com_om) / com_om * 100,
      res_rec = (recreational - deadN_4_om) / deadN_4_om * 100
    ) %>%
    pivot_longer(
      cols = c(res_com, res_rec),
      names_to = "type",
  ...
  # 3. Plotting
  title <- paste0("DeadN_", min_year, "_", max_year, "_", scenario_name, "_", model_run_type, "_Bias")
  ...
  p <- ggplot(plot_data, aes(om_name, mean_bias)) + 
    ...
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) + 
    labs(
      title = paste0("DeadN % Bias from ", min_year, "-", max_year, " - ", scenario_name),
      x = "OM name", 
      y = "% Bias"
    )
...
# #### Core 4
# 
# create_deadn_bias_plot(min_year, max_year_short_term, core_4, "Core 4")
# 
# #### All Years
# 
# create_deadn_bias_plot(min_year, max_year_short_term, all_years, "All years")

#### Selectivity rt_2

create_deadn_bias_plot_self_ref(min_year, max_year_short_term, selectivity_rt_2, "Selectivity 2 years")

#### Selectivity all_yrs

create_deadn_bias_plot_self_ref(min_year, max_year_short_term, selectivity_all_yrs, "Selectivity all years")
...
ggplot(plot_data, aes(x = year, y = med_val)) +
  geom_ribbon(aes(ymin = low, ymax = high, fill = om_name), alpha = 0.2) +
  geom_line(aes(color = om_name)) + 
  ggtitle("DeadB of Rec over time by OM") + ylab("DeadB (MT)")

plot_data <- summary$ts %>%
  filter(
    scenario %in% c(selectivity_rt_34),
    str_detect(model_run, "2116"),
    year >= 2017,
    year <= 2047
  ) %>%
  mutate(
...
ggplot(plot_data, aes(x = year, y = med_val)) +
  geom_ribbon(aes(ymin = low, ymax = high, fill = om_name), alpha = 0.2) +
  geom_line(aes(color = om_name)) + 
  ggtitle("DeadB of Rec over time by 2116 EM") 


# Pre-calculating
plot_data <- summary$ts %>%
  filter(
    scenario %in% c(selectivity_rt_2, "no_rt_x_rt_2"),
    str_detect(model_run, "OM"),
    year >= 2017,
    year <= 2047
  ) %>%
...
ggplot(plot_data, aes(x = year, y = med_val)) +
  ...
  ggtitle("DeadB of Rec over time of OM data by EM") + ylab("DeadB (MT)")


plot_data <- summary$ts %>%
  filter(
    scenario %in% c(selectivity_rt_34, "no_rt"),
    str_detect(model_run, "OM"),
    year >= 2017,
    year <= 2047
  ) %>%
  ...
  filter(om_name %in% c("flat", "reference")) %>%
  group_by(em_name, year) %>%
  summarise(
    med_val = median(SpawnBio),
    # Using Hmisc's logic for a 95% CI of the median
    low = Hmisc::smedian.hilow(SpawnBio, conf.int = 0.95)[2],
    high = Hmisc::smedian.hilow(SpawnBio, conf.int = 0.95)[3]
  )
...
ggplot(plot_data, aes(x = year, y = med_val)) +
  geom_ribbon(aes(ymin = low, ymax = high, fill = em_name), alpha = 0.2) +
  geom_line(aes(color = em_name)) + 
  ggtitle("SSB over time in the OM") + ylab("SpawnBio (MT)")

plot_data <- summary$dq %>%
  filter(
    scenario %in% c(selectivity_rt_2),
    str_detect(model_run, "OM"),
    year >= 2017,
    year <= 2047
  ) %>%
...
ggplot(plot_data, aes(x = year, y = med_val)) +
  geom_ribbon(aes(ymin = low, ymax = high, fill = om_name), alpha = 0.2) +
  geom_line(aes(color = om_name)) + 
  ggtitle("SSB over time in OM") + ylab("SpawnBio (MT)")


data <- EM_runs %>%
  filter(scenario %in% c(core_4, "rt_2_x_rt_2", "rt_2_x_no_rt")) %>%
  filter(
    str_detect(model_run, "2047"),
    year >= 2017,
    year <= 2047
  ) %>%
  rowwise()%>%
  ...
  summarise(
    med_val = median(mean_om_em),
    # Using Hmisc's logic for a 95% CI of the median
    low = Hmisc::smedian.hilow(mean_om_em, conf.int = 0.95)[2],
    high = Hmisc::smedian.hilow(mean_om_em, conf.int = 0.95)[3]
  )
  

# Plotting
ggplot(data, aes(x = year, y = med_val)) +
  geom_ribbon(aes(ymin = low, ymax = high, fill = scenario), alpha = 0.2) +
  geom_line(aes(color = scenario)) + 
  ggtitle("% Bias of SSB over time") + ylab("% Bias") +
  geom_vline(xintercept = c(2018, 2021), linetype = "dashed", color = "darkgrey")


data <- EM_runs %>%
  filter(scenario %in% c(selectivity_rt_34, "rt_2_x_rt_2", "rt_2_x_no_rt")) %>%
  filter(
    str_detect(model_run, "2047"),
    year >= 2017,
    year <= 2047

...
These outputs are generated from the results\_`{r} results_name` file. I took the SEDAR Red Grouper stock assessment and used SSMSE to introduce future red tide events in the OM and EM. There were `{r} length(scen_list)` total scenarios:
...
ssb_ts_plot <- summary$ts %>% 
  ...
  scale_color_manual(values = c("#D65F00", "black","#D65F00", "black","#D65F00", "black","#D65F00", "black","#D65F00", "black")) +
...
Plot of the max_grads by scenario and iteration.  
...
List of runs that have max_grad > 1.  
...
plot_variable_ts(stat_type = "mean"),
plot_variable_ts(variable = "SpawnBio", stat_type = "mean"),
plot_variable_ts(variable = "Bio_smry", stat_type = "mean"),
plot_variable_ts(variable = "F_1", stat_type = "mean"),
plot_variable_ts(variable = "F_2", stat_type = "mean"),
plot_variable_ts(variable = "F_4", stat_type = "mean"),
plot_variable_ts(variable = "F_5", stat_type = "mean")
)
...
This was filtered to just the final run.  
...
ratio_df <- left_join(EM_runs, OM_runs,
  ...
  mutate(across(
    ...
    .fns = ~ {
      em_col_name <- str_replace(cur_column(), "_om", "_em") # Get the name of the corresponding '_em' column
      em_value <- get(em_col_name) # Get the value from the '_em' column for the current row
      om_value <- .x # Get the value from the current '_om' column (represented by .x)
      # Conditional logic:
...
ratio_df_2 <- left_join(EM_runs, OM_runs,
                      by = c("year", "scenario", "iteration"),
                      suffix = c("_em", "_om")) %>%
  # EXCLUDE model_run_em from the pivoting process
  pivot_longer(
    cols = (contains("_em") | contains("_om")) & !contains("model_run"),
    names_to = c("metric", ".value"),
    names_pattern = "(.*)_(em|om)"
  ) %>%
  mutate(
    ratio = case_when(
      em == 0 & om == 0 ~ 1,
  ...
  select(year, scenario, iteration, model_run = model_run_em, metric, ratio) %>%
  mutate(metric = paste0(metric, "_ratio")) %>%
...
plot_f_ts <- function(scen) {
  ...
  fleet_ratios <- ratio_df %>%
    filter(
      ...
      str_detect(model_run, paste0("_EM_", end_yr))
    ) %>% 
...
residual_runs %>%
  ...
  geom_vline(xintercept = c(2017), color = "gray")+ # should this be the first predicted year?
...
residual_runs %>%
  ...
  filter(scenario %in% scen_list[1:5]) %>%
  ggplot(aes(year, value)) +
  geom_vline(xintercept = c(2005, 2014, 2018, 2021), linetype = "dashed", color = "gray") +
  geom_vline(xintercept = c(2017), color = "gray")+ # should this be the first predicted year?
  stat_summary(fun = mean, geom = "line") +
...
#### Long Term Residual Sums (2017-2050)

```{r}
residual_runs %>% 
  filter(year > 2017) %>%
  group_by(scenario) %>%
  reframe(sum_res_com = sum(res_com)/niter, sum_res_rec = sum(res_rec)/niter, sum_res_dead_5 = sum(res_dead_5)/niter) %>%
  kable(
    # Rename columns directly within kable
...
residual_runs_prop %>% 
  ...
  reframe(
    ...
    raw_total = (sum(res_com)/niter+sum(res_rec)/niter+sum(res_dead_5)/niter),
...
#### Short Term Residual Proportions (2017-2022) - New way?  
...
residual_runs_prop %>% 
  filter(year %in% seq(2017, 2022, 1)) %>%
  ggplot(aes(year, res_rec)) +
  geom_point() + 
  stat_summary(fun = mean, geom = "line") +
  stat_summary(fun.data = mean_cl_normal, geom = "ribbon", alpha = 0.3) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  facet_wrap(~scenario) + 
  ggtitle("Recreational Catch Residuals (2017-2022)") + xlab("Year") + ylab("Residuals")

residual_runs_prop %>% 
  filter(year %in% seq(2017, 2022, 1)) %>%
  ggplot(aes(year, res_com)) +
  geom_point() + 
  stat_summary(fun = mean, geom = "line") +
  stat_summary(fun.data = mean_cl_normal, geom = "ribbon", alpha = 0.3) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  facet_wrap(~scenario) + 
  ggtitle("Commercial Catch Residuals (2017-2022)") + xlab("Year") + ylab("Residuals")

residual_runs_prop %>% 
  filter(year %in% seq(2017, 2022, 1)) %>%
  ggplot(aes(year, res_dead_5)) +
  geom_point() + 
  stat_summary(fun = mean, geom = "line") +
  stat_summary(fun.data = mean_cl_normal, geom = "ribbon", alpha = 0.3) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  facet_wrap(~scenario) + 
  ggtitle("Red Tide Removal Residuals (2017-2022)") + xlab("Year") + ylab("Residuals")

residual_runs_prop %>% 
  filter(year %in% seq(2017, 2022, 1)) %>%
  group_by(scenario) %>%
  reframe(
    prop_com = mean(res_com)/mean(commercial)*100,
    prop_rec = mean(res_rec) / mean(deadB_4_om)*100,
    prop_red = mean(res_dead_5) /  mean(deadB_5_om)*100,
    raw_total = mean(res_com)+mean(res_rec)+mean(res_dead_5),
    raw_prop = (mean(res_com)+mean(res_rec)+mean(res_dead_5)) /  (mean(com_om)+mean(deadB_4_om) + mean(deadB_5_om)) *100
  ) %>% 
  kable(
    # Rename columns directly within kable
    col.names = c("Scenario", "Commercial Catch Average Residuals (%)", "Recreational Catch Average Residuals (%)", "Red Tide Discards Average Residuals (%)", "Total Removals Average Residuals (MT)", "Proportion of Average Residuals (%)"),
    align = c("l", "c", "c", "c", "c", "c"), # Align columns (left, center, center, center)
    digits = 2
  ) %>%
  kable_styling(
...
```{r}
residual_runs_prop %>% 
  filter(year %in% seq(2017, 2047, 1)) %>%
  ggplot(aes(year, res_rec)) +
  geom_point() + 
  stat_summary(fun = mean, geom = "line") +
  stat_summary(fun.data = mean_cl_normal, geom = "ribbon", alpha = 0.3) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  facet_wrap(~scenario) + 
  ggtitle("Recreational Catch Residuals (2017-2047)") + xlab("Year") + ylab("Residuals") + 
  ylim(-10000,10000)
...
residual_runs_prop %>% 
  ...
  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  facet_wrap(~scenario) + 
  ggtitle("Commercial Catch Residuals (2017-2047)") + xlab("Year") + ylab("Residuals")+ 
  ylim(-10000,10000)
...
residual_runs_prop %>% 
  ...
  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  facet_wrap(~scenario) + 
  ggtitle("Red Tide Removal Residuals (2017-2047)") + xlab("Year") + ylab("Residuals")+ 
  ylim(-10000,10000)
...
OM_means <- summary$ts %>%
  filter(model_run == str_detect(model_run, "_OM")) %>%
...
EM_means_re <- summary$ts %>%
  filter(str_detect(model_run, paste0("_EM_", end_yr))) %>%
  rowwise() %>%
  mutate(
...
#   mutate(r0 = exp(SR_LN_R0)) %>%
#   select(!SR_LN_R0) %>%
#   filter(str_detect(model_run, paste0("_OM|_EM_", end_yr))) %>%
#   pivot_wider(
#       names_from = model_run,  # this will create "OM" and "EM" columns
#       values_from = r0
#   ) %>%
#   rename_with(~ "OM", .cols = contains("_OM")) %>%
#   rename_with(~ "EM", .cols = contains(paste0("_EM_", end_yr))) %>%
...
summary$ts %>%
  filter(str_detect(model_run, paste0("_OM|_EM_", end_yr)), year %in% c(2005, 2014, 2018, 2021)) %>%
  ggplot(aes(factor(year), F_5)) + 
  geom_boxplot(aes(color = model_run)) + 
  facet_wrap(~scenario) +
  ggtitle(paste0("model_run = ", end_yr))
```
...
```{r}
summary$ts %>%
  filter(str_detect(model_run, paste0("_OM|_EM_", 2023)), year %in% c(2005, 2014, 2018, 2021)) %>%
  ggplot(aes(factor(year), F_5)) + 
  geom_boxplot(aes(color = model_run)) + 
  facet_wrap(~scenario) +
...
### end_yr More variables, zoomed in

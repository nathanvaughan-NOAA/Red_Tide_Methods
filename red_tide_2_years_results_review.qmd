...
results_name <- "_rt_2_uncertainty"
...
f_ts_plots[[4]]
```
### 2 years red tide OM X 2 years red tide EM  Fixed

```{r, warning=FALSE}
f_ts_plots[[5]]
```

### No red tide OM x all years red tide EM 

```{r, warning=FALSE}
f_ts_plots[[6]]
```

### 2 years red tide OM x all years red tide EM 

```{r, warning=FALSE}
f_ts_plots[[7]]
```

### Every 3 years red tide OM x all years red tide EM 
...
ratio_df <- 
  left_join(EM_runs, OM_runs,
                      by = c("year", "scenario", "iteration"),
                      suffix = c("_em", "_om")) %>%
  mutate(F_om = F_1_om + F_2_om + F_3_om + F_4_om + F_5_om, 
         F_em = F_1_em + F_2_em + F_3_em + F_4_em + F_5_em) %>%
  mutate(across(
    .cols = where(is.numeric) & ends_with("_om"),
    .fns = ~ {
      # Get the name of the corresponding '_em' column
...
plot_f_ts <- function(scen) {
  ...
  rt_summary <- fleet_ratios %>%
    ...
    # Calculate the median and the 25th/75th percentiles for the ribbon
    reframe(
      median_ratio = median(F_ratio, na.rm = TRUE),
      lower_quartile = quantile(F_ratio, 0.25, na.rm = TRUE),
      upper_quartile = quantile(F_ratio, 0.75, na.rm = TRUE)
    )
  
  
  rt_terminal_ratio <- fleet_ratios %>%
    ggplot() +
    geom_point(aes(y = F_ratio, year),
               color = "grey",
               alpha = 0.5) + # Individual points
    # Add the error ribbon using the summary data
    geom_ribbon(
...
ifelse(
        ...
        ifelse(grepl("all", scenario), "no_rt_x_all_yrs", "2 Years Red Tide")
      )
    ),
    om_model = ifelse(
      scenario %in% c("no_rt", "no_rt_x_rt_2", "no_rt_x_rt_2_high", "no_rt_x_all_yrs"),
      "No Red Tide",
      ifelse(
        grepl("high", scenario),
        "2 Years High Red Tide",
...
mutate(
    em_model = ifelse(
      scenario %in% c("no_rt", "rt_2_x_no_rt", "rt_2_x_no_rt_high", "no_rt_x_all_yrs"),
      "No Red Tide",
      ifelse(
        grepl("high", scenario),
        "2 Years High Red Tide",
...
EM_means_ratio <- summary$ts %>%
  ...
  group_by(scenario, iteration, year) %>%
  mutate(
    # Check for NA or if the denominator is zero.
    re_Recruit_0 = ifelse(
      is.na(Recruit_0) | is.na(OM_Recruit_0) | OM_Recruit_0 == 0,
      1,
      Recruit_0/OM_Recruit_0
    ),
    re_F_5 = ifelse(
      is.na(F_5) | is.na(OM_F_5) | OM_F_5 == 0,
      1,
      F_5/OM_F_5
    ),
    re_SpawnBio = ifelse(
      is.na(SpawnBio) | is.na(OM_SpawnBio) | OM_SpawnBio == 0,
      1,
      SpawnBio/OM_SpawnBio
    ),
    re_com = ifelse(
      is.na(commercial) | is.na(OM_com) | OM_com == 0,
      1,
      commercial/OM_com
    ),
    re_rec = ifelse(
      is.na(recreational) | is.na(OM_rec) | OM_rec == 0,
      1,
      recreational/OM_rec
    ),
    re_dead_5 = ifelse(
      is.na(deadB_5) | is.na(OM_dead_5) | OM_dead_5 == 0,
      1,
      deadB_5/OM_dead_5
    ),
    re_com_f = ifelse(
      is.na(com_f) | is.na(OM_com_f) | OM_com_f == 0,
      1,
      com_f/OM_com_f
    ),
    re_rec_f = ifelse(
      is.na(rec_f) | is.na(OM_rec_f) | OM_rec_f == 0,
      1,
      rec_f/OM_rec_f
    ),
    re_abundance = ifelse(
      is.na(Bio_smry) | is.na(OM_abundance) | OM_abundance == 0,
      1,
...
### Tabset of Quarterly Sales Tables 
...
# OM_lines should work well for this because it's the mean/median for each variable by year and scenario.  

# We then want to see if the EM_runs value is within 10% of the OM_lines value in the scenario and year combo.  

threshold_table <- EM_runs %>%
  left_join(OM_lines, join_by(year, scenario)) %>%
  mutate(
    # 1. Calculate the 10% tolerance band based on OM_value
    lower_bound = F_5_mean * 0.90,
    upper_bound = F_5_mean * 1.10,
    
    # 2. Evaluate the condition
    is_within_10_percent = (F_5 >= lower_bound) & (F_5 <= upper_bound),
    
    # Optional: Calculate the actual percentage difference (for reporting)
    percent_diff = (F_5 - F_5_mean) / F_5_mean * 100
  ) %>%
  # Select key columns for easy viewing
  select(model_run, year, scenario, F_5_mean, F_5, is_within_10_percent, percent_diff)
  
threshold_table %>%
  group_by(scenario, year) %>%
  filter(model_run == "default_sigmaR_EM_2047") %>%
  count(is_within_10_percent) %>%
  rename(count = n) %>%
  ungroup() %>%
  group_by(scenario, is_within_10_percent) %>%
  summarise(scenario_sum = sum(count)) %>%
  kable(
    # Rename columns directly within kable
    align = c("l", "c", "c", "c") # Align columns (left, center, center, center)
  ) %>%
...
threshold_table %>% 
  filter(year %in% c(2018, 2021), model_run == "default_sigmaR_EM_2047") %>%
  group_by(scenario, year) %>%
  count(is_within_10_percent) %>%
  rename(count = n) %>%
  ungroup() %>%
  group_by(year, scenario, is_within_10_percent) %>%
  summarise(scenario_sum = sum(count))%>%
  pivot_wider(
    names_from = is_within_10_percent,  
    values_from = scenario_sum,          
    names_prefix = "is_within_"          # Optional: Adds a prefix to the new column names
  ) %>%
  # Optional: Rename the columns to match your desired 'TRUE' and 'FALSE' format
  rename(
    'Is within 10%' = is_within_TRUE,
    'Is outside 10%' = is_within_FALSE
  ) %>%
  kable(
    # Rename columns directly within kable
    align = c("l", "c", "c", "c") # Align columns (left, center, center, center)
...
summary$ts %>%
  filter(model_run %in% c("default_sigmaR_OM", "default_sigmaR_EM_2047")) %>%
...
summary$ts %>%
  filter(model_run %in% c("default_sigmaR_OM", "default_sigmaR_EM_2047"), year %in% c(2005, 2014, 2018, 2021)) %>%
  ggplot(aes(factor(year), F_5)) + 
  geom_boxplot(aes(color = model_run)) + 
  facet_wrap(~scenario) +
...
summary$ts %>%
  filter(model_run %in% c("default_sigmaR_OM", "default_sigmaR_EM_2023"), year %in% c(2005, 2014, 2018, 2021)) %>%
  ggplot(aes(factor(year), F_5)) + 
  geom_boxplot(aes(color = model_run)) + 
  facet_wrap(~scenario) +
...
summary$ts %>%
  ...
  # Optional: Rename the columns to match your desired 'TRUE' and 'FALSE' format
  rename(
    'EM_2047' = default_sigmaR_EM_2047,
    'OM' = default_sigmaR_OM
  ) %>%
  kable(
    # Rename columns directly within kable
    align = c("l", "c", "c", "c"), # Align columns (left, center, center, center)
...
abs_error <- EM_runs %>%
  ...
  group_by(scenario, year) %>%
  reframe(
    mae_Recruit_0 = sum(ae_Recruit_0)/n_iterations,
    mae_F_5 = sum(ae_F_5)/n_iterations,
    mae_SpawnBio = sum(ae_SpawnBio)/n_iterations,
    mae_com = sum(ae_com)/n_iterations,
    mae_rec = sum(ae_rec)/n_iterations,
    mae_dead_5 = sum(ae_dead_5)/n_iterations,
    mae_abundance = sum(ae_abundance)/n_iterations
  )

abs_error %>%
  # Pivot the data longer
  pivot_longer(
    cols = c(mae_com, mae_rec, mae_dead_5),
    names_to = "variable",
    values_to = "value",
    names_transform = list(
      variable = ~recode(
        .x,
        "mae_com" = "Commercial Catch",
        "mae_rec" = "Recreational Catch",
        "mae_dead_5" = "Red Tide Dead"
      )
  )) %>%
  ggplot(aes(year, value)) +
  geom_vline(xintercept = c(2005, 2014, 2018, 2021), linetype = "dashed", color = "gray") +
  ...
  #coord_cartesian(ylim = c(0,3)) +
  #coord_cartesian(xlim = c(2017, 2025)) +
  labs(color = "EM", fill = "EM", x = "Year", y = "Mean Absolute Error")
...
smape <- function(em, om) {
  abs(em-om)/(abs(om)+abs(em))
}
...
abs_error <- EM_runs %>%
  ...
  group_by(scenario, iteration, year) %>%
  mutate(
    ae_Recruit_0 = smape(Recruit_0_em, Recruit_0_om),
    ae_F_5 = smape(F_5_em,F_5_om),
    ae_SpawnBio = smape(SpawnBio_em,SpawnBio_om),
    com_om = sum(retainB_1_om, retainB_2_om),
    ae_com = smape(commercial, com_om),
    ae_rec = smape(recreational, retainB_4_om),
    ae_dead_5 = smape(deadB_5_em, deadB_5_om),
    ae_abundance = smape(Bio_smry_em, Bio_smry_om)
  ) %>%
  group_by(scenario, year) %>%
  reframe(
    mae_Recruit_0 = sum(ae_Recruit_0)*2/n_iterations,
    mae_F_5 = sum(ae_F_5)*2/n_iterations,
    mae_SpawnBio = sum(ae_SpawnBio)*2/n_iterations,
    mae_com = sum(ae_com)*2/n_iterations,
    mae_rec = sum(ae_rec)*2/n_iterations,
    mae_dead_5 = sum(ae_dead_5)*2/n_iterations,
    mae_abundance = sum(ae_abundance)*2/n_iterations
  )
...
abs_error %>%
  ...
  #coord_cartesian(ylim = c(0,3)) +
  #coord_cartesian(xlim = c(2017, 2025)) +
  labs(color = "EM", fill = "EM", x = "Year", y = "SMAPE")

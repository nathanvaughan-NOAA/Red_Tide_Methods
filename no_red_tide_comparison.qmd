...
file.copy(file.path(OM_no_red_tide_path, "data.ss_new"), file.path(OM_no_red_tide_path, "data_echo.ss_new"))
...
ctl<-r4ss::SS_readctl(file.path(OM_no_red_tide_path, "red_grouper_1986_2017_RedTideFleet.ctl"))
# str(ctl)
# names(ctl)
# rownames(ctl$SR_parms)

#dat<-r4ss::SS_readdat(file.path(OM_red_tide_path, "red_grouper_1986_2017_RedTideFleet.dat"))
dat<-r4ss::SS_readdat(file.path(OM_no_red_tide_path, "red_grouper_1986_2017_RedTideFleet.dat"))
# names(dat)
...
sample_struct_list_all <- list("no_red_tide_struct" = sample_struct_no_red_tide)
#sample_struct_list_all <- list("no_red_tide" = sample_struct)
#sample_struct_list_all <- list("red_tide" = sample_struct)
...
fore<-r4ss::SS_readforecast(file=file.path(OM_no_red_tide_path, "forecast.ss"), verbose=FALSE)
fore$Forecast
...
res<-run_SSMSE(
  scen_name_vec=c("no_red_tide_struct"), # name of the scenario
  out_dir_scen_vec = run_res_path, # directory in which to run the scenario
  iter_vec = c(2), # run with 3 iterations each
  OM_name_vec = NULL, # specify directories instead
  OM_in_dir_vec = c(normalizePath(OM_no_red_tide_path)), # OM files
  EM_name_vec = NULL, # specify directories instead
  EM_in_dir_vec = c(normalizePath(EM_no_red_tide_path)), # EM files
  run_EM_last_yr = TRUE,
...
```{r}
dir <- "C:\\Users\\apn26\\Documents\\CIMAS\\Red_Tide_Methods\\run_SSMSE_no_red_tide\\results\\no_red_tide_struct\\5\\OM_no_red_tide_OM"
bin <- "C:/Users/apn26/Documents/SSMSE/inst/bin/Windows64/ss.exe"

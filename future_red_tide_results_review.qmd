...
Plotting the same ratios as above but by the "time from the terminal year" instead of year.  
...
Figure 10. Term plots are meant to be a "short term" (2000-2030) and "long term" (1989-2067) look at a few key parameters from the time series dataset.  These plots demonstrate that red tide events correspond with SPR Ratio spikes, and can slowly decrease retained biomass or spawning biomass in the short term.  In the long term, there are no trends.  
...
Figure 11. Management Term plots are meant to be a "short term" (2000-2027) and "long term" (1989-2067) look at a few key parameters from the time series dataset.  These plots demonstrate that red tide events correspond with SPR Ratio spikes, and can slowly decrease retained biomass or spawning biomass in the short term.  In the long term, there are no trends.  I tried to add median trend lines for the OM and EM but they aren't very clear because of how the lines are colored.  
...
```{r}
plot_rt_ratio_ts <- function(scen) {
...
rt_summary <- red_tide_means %>%
  group_by(year) %>%
  reframe(
    median_ratio = median(ratio_F_5, na.rm = TRUE),
...
rt_terminal_ratio <- red_tide_means %>%
  ggplot() +
  geom_point(aes(year, ratio_F_5), color = "grey") +
  #stat_summary(fun = mean, geom = "line", linewidth = 1) +
  geom_ribbon(data = rt_summary, aes(x = year, ymin = ymin, ymax = ymax),     # Ribbon for ±1 SD
              fill = "lightblue", alpha = 0.3) +
  geom_line(data = rt_summary, aes(x = year, y = median_ratio),   # median line
            linewidth = .7, color = "black") +
  geom_hline(yintercept = 1, linetype = "dashed")+
  scale_color_nmfs(palette = "oceans", reverse = TRUE) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  ggtitle(scen) +
  ylab("Ratio of F_5 (EM:OM)") + xlab("Years") +
  theme_bw()
...
rt_ratio_ts_plots <- map(scen_list, plot_rt_ratio_ts)

```

## F_5 ratio over time, all model runs

Only years with red tide events, exclude OMs, all model_runs.  
...
Figure 12. Median Ratio of F_5 over time in years with red tide.  
...
red_tide_means <- summary$ts %>% 
  filter(year >= 2017, 
         scenario == scen, 
         year %in% red_tide_years, 
         model_run == paste0(OM_name, "_EM_2065"), 
         !str_detect(model_run, "OM")) %>%
  mutate(ratio_F_5 = F_5/OM_mean, 
         end_year = as.numeric(str_extract(model_run, "\\d{4}$")),
         years_until_terminal = end_year - year)
...
## F_5 ratio over time, EM_2065

Only years with red tide events, exclude OMs, Just EM_2065
...
ratio_df <- left_join(EM_runs, OM_runs, 
  ...
  mutate(across(
    .cols = where(is.numeric) & ends_with("_om"), 
    .fns = ~ get(str_replace(cur_column(), "_om", "_em")) / .x, 
    .names = "{str_remove(.col, '_om')}_ratio"
  )) %>%
  select(year, scenario, iteration, model_run = model_run_em, ends_with("_ratio"))
...
summary_stats <- ratio_df %>%
  # Use the same filters as your plot
  filter(scenario == scen, !is.na(F_5_ratio)) %>%
  # Group by year to calculate stats for each year
  group_by(year) %>%
  # Calculate the median and the 25th/75th percentiles for the ribbon
  summarise(
    median_ratio = median(F_5_ratio),
    lower_quartile = quantile(F_5_ratio, 0.25),
    upper_quartile = quantile(F_5_ratio, 0.75)
  )

rt_terminal_ratio <- ratio_df %>%
  filter(scenario == scen, !is.na(F_5_ratio)) %>%
  ggplot(aes(x = year)) + # Move x aesthetic to the main ggplot call
  geom_point(aes(y = F_5_ratio), color = "grey", alpha = 0.5) + # Individual points
  # Add the error ribbon using the summary data
...
## F_5 ratio over time, all models cleaner method

I used a different method to calculate the ratios and the ribbons use 25 and 75% quantiles  This has all model_runs and is made by joining each EM to an OM by iteration, scenario, and year.  

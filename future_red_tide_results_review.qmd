...
res <- readRDS(file = file.path("C:/Users/apn26/Documents/CIMAS/Personal Notes/cc_storage/", "results_multirun_morality_cpuefix.rda"))
summary <- readRDS(file = file.path("C:/Users/apn26/Documents/CIMAS/Personal Notes/cc_storage/", "results_summary_multirun_morality_cpuefix.rda"))
...
rt_terminal_ratio <- red_tide_means %>%
  ggplot(aes(years_until_terminal, ratio_F_5)) +
...
not_sure_plots <- map(scen_list, plot_rt_terminal_ratio)

```

## Fishing terminal year plots all years

Plots of all years instead of just red tide years
...
red_tide_means <- summary$ts %>% 
  filter(year >= 2017, 
         ...
         !str_detect(model_run, "init")) %>%
  left_join(OM_mean, by = c("year")) %>%
  mutate(ratio_Rec = ifelse(is.na(rec_dev/OM_value), 0, rec_dev/OM_value), #this put zeros back in.  
         end_year = as.numeric(str_extract(model_run, "\\d{4}$")),
         years_until_terminal = end_year - year)
...
rt_terminal_ratio <- red_tide_means %>%
  ...
  xlim(0,50) +
  ggtitle(scen) +
  ylab("Ratio of RecDev (EM:OM)") + xlab("Years until terminal") + 
  theme_bw()
...
## Terminal year rec_devs ratios, all years
...
## Terminal year rec_devs ratios, red tide years
...
OM_means <- summary$ts %>%
  filter(year >= 2017, model_run == "default_OM") %>%
...
I was able to put cpue from one scenario/iteration but it doesn't get projected past 2017.

```{r, include=FALSE}

# where are the indexes?  
# the indexes are not currently extracted they are in the Report file under INDEX 2
# I would need to have code that pulls the observed and expected values from the Report file
# See if that is an option I can add to the summary or if I need to manually pull from
# all of the files.  


indecies <- readRDS(file = file.path("C:/Users/apn26/Documents/CIMAS/Personal Notes/cc_storage/testing_cloud/cleaned_results", "results_multirun_morality_2.rda"))

library(r4ss)
red_tide_reg_3_1_2065 <- SS_output("C:/Users/apn26/Documents/CIMAS/Personal Notes/cc_storage/results_multirun_morality_cpuefix/red_tide_regular_3/1/default_EM_2047")
```

```{r}
red_tide_reg_3_1_2065$cpue %>%
  filter(Fleet_name %in% c("commHL", "commLL", "Rec"))%>%
  ggplot(aes(Yr, Obs)) +
  geom_line(aes()) +
  geom_line(aes(Yr, Exp), linetype = "dashed") +
  facet_grid(rows = vars(Fleet_name))
